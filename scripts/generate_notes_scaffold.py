...
从一个零基础读者会自然产生的问题开始：这一节到底要解释什么？为什么教材要在这里讲它？优先使用主线教材的开场问题、生活场景或历史背景进入，不要一上来就抽象总结。

## 正文写作提示

正文不要机械套模板标题，应按内容自然分节。写法应像教材带读，而不是主题总结。推荐顺序是：问题进入、原书场景或例子、概念出现、逻辑展开、例子校准、必要的结构化整理。写作时必须先对照主线教材的小节顺序，按原书的展开方式组织内容，不要把三本书强行融合成新的框架。
